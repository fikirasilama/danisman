...
function initScene0(cv: HTMLCanvasElement, m: MRef, active: ARef) {
  ...
  function draw() {
    ...
    blobs.forEach(b => {
      ...
      g.addColorStop(0, `rgba(74,148,212,${b.op})`)
      g.addColorStop(0.5, `rgba(40,110,180,${b.op * 0.4})`)
    ...
    stars.forEach(s => {
      ...
      const col  = s.cream ? `rgba(216,236,248,${op})` : `rgba(102,178,245,${op})`
      ...
      if (!s.reset && s.px !== 0) {
        ...
        ctx.strokeStyle = s.cream ? `rgba(216,236,248,${trailOp})` : `rgba(102,178,245,${trailOp})`
...
function initScene1(cv: HTMLCanvasElement, m: MRef, active: ARef) {
  ...
  function drawFlare(x: number, y: number, r: number, op: number) {
    [0, Math.PI / 4, Math.PI / 2, 3 * Math.PI / 4].forEach(angle => {
      ...
      ctx.strokeStyle = `rgba(216,236,248,${op * 0.35})`
      ctx.lineWidth = 0.6
      ctx.stroke()
    })
  }
  ...
  function draw() {
    ...
    neb.addColorStop(0.6, "rgba(6,18,42,0.08)")
    neb.addColorStop(1, "rgba(6,24,62,0.22)")
    ...
    ring.forEach(rs => {
      ...
      ctx.fillStyle = `rgba(74,148,212,${ro})`
    ...
    stars.forEach((a, i) => {
      stars.slice(i + 1).forEach((b, j) => {
        ...
        g.addColorStop(0, `rgba(74,148,212,${op})`)
        g.addColorStop(0.5, `rgba(102,178,245,${op * 1.3})`)
        g.addColorStop(1, `rgba(74,148,212,${op})`)
    ...
    stars.forEach((s, i) => {
      ...
      if (s.anchor) {
        ...
        ag.addColorStop(0, `rgba(216,236,248,${op * 0.9})`)
        ag.addColorStop(0.3, `rgba(102,178,245,${op * 0.5})`)
        ag.addColorStop(1, "rgba(0,0,0,0)")
        ctx.fillStyle = ag; ctx.beginPath(); ctx.arc(x, y, r * 5, 0, Math.PI * 2); ctx.fill()
        drawFlare(x, y, r, op * tw)
      }
      ctx.fillStyle = s.anchor ? `rgba(216,236,248,${op})` : `rgba(102,178,245,${op})`
...
function initScene2(cv: HTMLCanvasElement, m: MRef, active: ARef): () => void {
  ...
  function draw() {
    ...
    cols.forEach(col => {
      ...
      if (col.y - col.lastY >= FS) {
        ...
        if (col.y >= 0 && col.y <= cv.height) {
          ctx.shadowColor = bursting ? "#ffffff" : "#9ad8ff"
          ctx.shadowBlur  = bursting ? 28 : 10
          ctx.fillStyle   = bursting ? "#ffffff" : "#9ad8ff"
    ...
    for (let i = drops.length - 1; i >= 0; i--) {
      ...
      ctx.shadowBlur  = 14
      ctx.fillStyle   = `rgba(154,216,255,${d.op})`
...
function initScene3(cv: HTMLCanvasElement, m: MRef, active: ARef) {
  ...
  function draw() {
    ...
    cells.forEach(({ col, row, depth, sx, sy }) => {
      ...
      if (col < GN - 1) {
        ...
        if (pr) {
          ...
          ctx.strokeStyle = `rgba(74,148,212,${lineOp})`
      ...
      if (isB) {
        ...
        ng.addColorStop(0, `rgba(102,178,245,${fog * (0.7 + pulse * 0.3)})`)
        ng.addColorStop(1, "rgba(0,0,0,0)")
        ctx.fillStyle = ng; ctx.beginPath(); ctx.arc(sx, sy, 16 * fog, 0, Math.PI * 2); ctx.fill()
        ctx.fillStyle = `rgba(160,228,255,${fog})`
        ctx.beginPath(); ctx.arc(sx, sy, 3.8 * fog, 0, Math.PI * 2); ctx.fill()
      } else if (glow > 0.28) {
        ctx.fillStyle = `rgba(74,148,212,${fog * glow * 0.55})`
        ctx.beginPath(); ctx.arc(sx, sy, 1.5, 0, Math.PI * 2); ctx.fill()
      }
    })

    // Beacon connection (dashed)
    ctx.setLineDash([4, 9]); ctx.strokeStyle = "rgba(74,148,212,0.30)"; ctx.lineWidth = 0.9
    ctx.beginPath()
    bwp.forEach((w, i) => {
      const p = proj(w.x, w.y, w.z)
      if (!p) return
    ...
    if (tp) {
      ...
      tg.addColorStop(0, "rgba(216,236,248,1)"); tg.addColorStop(1, "rgba(74,148,212,0)")
    ...
    BEACONS.forEach((b, bi) => {
      ...
      ctx.strokeStyle = "rgba(74,148,212,0.28)"; ctx.lineWidth = 0.75; ctx.stroke()
      ...
      if (p1) {
        ctx.fillStyle = "rgba(216,236,248,0.92)"
      ...
      if (p2) {
        ctx.fillStyle = "rgba(74,148,212,0.78)"
...
function initScene4(cv: HTMLCanvasElement, section: HTMLElement | null, m: MRef, active: ARef) {
  ...
  function drawFlare(x: number, y: number, r: number, op: number) {
    [0, 0.52, 1.05, 1.57, 2.09, 2.62].forEach((a, i) => {
      ...
      ctx.strokeStyle = `rgba(216,236,248,${op * 0.28})`
  ...
  function draw() {
    ...
    hg.addColorStop(0, `rgba(102,178,245,${0.25 + Math.sin(t * 1.8) * 0.1})`)
    ...
    for (let i = waves.length - 1; i >= 0; i--) {
      ...
      if (w.shock) {
        ...
        ctx.strokeStyle = `rgba(216,236,248,${w.op * 0.9})`
        ctx.lineWidth = 3.5; ctx.stroke()
        ctx.beginPath(); ctx.arc(w.x, w.y, w.r * 0.88, 0, Math.PI * 2)
        ctx.strokeStyle = `rgba(102,178,245,${w.op * 0.5})`
        ctx.lineWidth = 1.5; ctx.stroke()
      } else {
        ctx.beginPath(); ctx.arc(w.x, w.y, w.r, 0, Math.PI * 2)
        ctx.strokeStyle = `rgba(74,148,212,${w.op})`
    ...
    for (let i = spirals.length - 1; i >= 0; i--) {
      ...
      sg.addColorStop(0, `rgba(216,236,248,${s.op})`)
      sg.addColorStop(1, "rgba(74,148,212,0)")
...
// ─── static data ──────────────────────────────────────────────────────────────
const HERO_WORDS = ["FİKİR", "AŞISI", "yaptırdınız", "mı?"]
const SERVICES = [
...
function Vignette() {
  return (
    <div className="absolute inset-0 pointer-events-none z-[1]"
      style={{ background: "radial-gradient(ellipse 90% 80% at 50% 50%, transparent 20%, rgba(6,8,12,0.78) 100%)" }} />
  )
}
...
export default function Home() {
  ...
  useGSAP(() => {
    ...
    gsap.to(".word-inner", {
      textShadow: "0 0 30px rgba(102,178,245,0.7), 0 0 60px rgba(74,148,212,0.35)",
  ...
  const scrollTo = (i: number) => {
    window.scrollTo({ top: i * 1.65 * window.innerHeight, behavior: "smooth" })
  }
  ...
  return (
    <>
      ...
      <div ref={cursorDot} className="fixed z-[9999] pointer-events-none rounded-full"
        style={{ width: 7, height: 7, backgroundColor: "#66b2f5", transform: "translate(-50%,-50%)" }} />
      <div ref={cursorRing} className="fixed z-[9998] pointer-events-none rounded-full"
        style={{ width: 38, height: 38, border: "1px solid rgba(102,178,245,0.5)", transform: "translate(-50%,-50%)" }} />

      {/* Scene dots */}
      <div ref={dotsRef} className="fixed right-5 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-3">
        {[0,1,2,3,4].map(i => (
          <button key={i} onClick={() => scrollTo(i)}
            style={{ width: 8, height: 8, padding: 0, cursor: "none", border: "1px solid #4a95d4",
      ...
      <div ref={containerRef} style={{
        ...
        backgroundImage: "radial-gradient(ellipse 140% 60% at 50% 100%, rgba(4,12,24,1) 0%, #060810 55%)"
      }}>
        ...
        <section ref={s0} className="relative h-screen overflow-hidden">
          ...
          <div className="relative z-10 h-full flex flex-col justify-center px-12 md:px-20 max-w-5xl">
            <p className={`s0-meta ${it.className}`}
              style={{ fontSize: "0.82rem", letterSpacing: "0.26em", color: "#6f7e8a",
                ...
                <span key={i} style={{ display: "inline-block", overflow: "hidden",
                  ...
                  <span className={`word-inner ${cg.className}`}
                    style={{ display: "block",
                      ...
                      color: "#c9d8e8", letterSpacing: "0.04em",
                      fontStyle: i === 2 || i === 3 ? "italic" : "normal" }}>
                    {word}
                  </span>
                </span>
              ))}
            ...
            <p className={`s0-sub ${it.className}`}
              style={{ marginTop: "2rem", fontSize: "clamp(1rem, 1.5vw, 1.125rem)",
                color: "#6f7e8a", maxWidth: 520, lineHeight: 1.75, fontWeight: 300 }}>
              Yapay zekadan korkan işletmeler geride kalıyor.
              Biz onu içinize işliyoruz.
            </p>
          </div>
            ...
            <div className="flex flex-col gap-6">
              {["Sade ama derin", "Sektörünüze konuşur", "Bir el sıkışma"].map((item, i) => (
                <div key={i} className={`s1-item ${it.className} flex items-center gap-4`}>
                  <span style={{ width: 28, height: 1, backgroundColor: "#4a95d4", flexShrink: 0 }} />
                  <span style={{ fontSize: "1.875rem", color: "#c9d8e8", fontWeight: 300, letterSpacing: "0.04em", lineHeight: 1.3 }}>
                    {item}
                  </span>
                </div>
              ))}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-px" style={{ maxWidth: 740 }}>
              {SERVICES.map(s => (
                <div key={s.name} className="s2-card px-6 py-5"
                  style={{ backgroundColor: "rgba(6,8,12,0.72)", border: "1px solid rgba(74,148,212,0.12)" }}>
                  <p className={cg.className}
                    style={{ fontSize: "2rem", fontWeight: 400, color: "#c9d8e8", marginBottom: "0.4rem" }}>
                    {s.name}
                  </p>
                  <p className={it.className}
                    style={{ fontSize: "1rem", color: "#6f7e8a", letterSpacing: "0.05em", lineHeight: 1.7 }}>
                    {s.desc}
                  </p>
                </div>
              ))}
              ...
              {STEPS.map(s => (
                <div key={s.num} className="s3-step flex gap-6 items-start">
                  <span className={cg.className}
                    style={{ fontSize: "2.6rem", fontWeight: 300, color: "#4a95d4", lineHeight: 1, minWidth: 50 }}>
                    {s.num}
                  </span>
                  <div>
                    <p className={cg.className}
                      style={{ fontSize: "2rem", fontWeight: 400, color: "#c9d8e8", marginBottom: "0.3rem" }}>
                      {s.label}
                    </p>
                    <p className={it.className}
                      style={{ fontSize: "1.0625rem", color: "#6f7e8a", lineHeight: 1.7, fontWeight: 300 }}>
                      {s.desc}
                    </p>
                  </div>
                </div>
        ...
        <section ref={s4} className="relative h-screen overflow-hidden flex items-center justify-center">
          ...
          <div className="relative z-10 text-center px-8 max-w-2xl">
            <p className={`s4-quote ${it.className}`}
              style={{ fontSize: "0.82rem", letterSpacing: "0.28em", color: "#6f7e8a",
                textTransform: "uppercase", marginBottom: "2rem" }}>
            ...
            <h2 className={`s4-cta ${cg.className}`}
              style={{ fontSize: "clamp(2.2rem, 4.8vw, 4.2rem)", fontWeight: 300, fontStyle: "italic",
                color: "#d8eaf4", lineHeight: 1.2, marginBottom: "3rem" }}>
              İşinizin sessiz katmanı artık zeki olabilir.
            </h2>
            <button
              className={`s4-btn ${it.className}`}
              style={{ border: "1px solid #4a95d4", color: "#4a95d4", backgroundColor: "transparent",
                padding: "0.9rem 2.4rem", fontSize: "0.7rem", letterSpacing: "0.22em",
                textTransform: "uppercase", cursor: "none" }}
              onMouseEnter={e => gsap.to(e.currentTarget, { backgroundColor: "#4a95d4", color: "#060810", duration: 0.25 })}
              onMouseLeave={e => gsap.to(e.currentTarget, { backgroundColor: "transparent", color: "#4a95d4", duration: 0.25 })}
            >
              Keşif Görüşmesi
            </button>
          </div>
      ...
      <button
        ...
        style={{
          ...
          border: `1px solid rgba(102,178,245,${playing ? 0.8 : 0.4})`,
          backgroundColor: "rgba(102,178,245,0.1)",
          backdropFilter: "blur(12px)",
          color: "#66b2f5", display: "flex", alignItems: "center", justifyContent: "center",
      ...
      <style>{`
        @keyframes audioGlow {
          0%   { box-shadow: 0 0 6px 0px rgba(102,178,245,0.35); }
          50%  { box-shadow: 0 0 16px 6px rgba(102,178,245,0.15); }
          100% { box-shadow: 0 0 6px 0px rgba(102,178,245,0.35); }
        }
      `}</style>
    </>
  )
